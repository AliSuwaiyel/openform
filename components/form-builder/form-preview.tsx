...
export function FormPreview({
  ...
  if (questions.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-[400px] p-8" dir="rtl">
        <div className="text-center">
          <p style={{ color: theme.textColor }} className="opacity-50">
            أضف أسئلة لرؤية المعاينة
          </p>
        </div>
      </div>
    )
    ...
    <div className="p-8 space-y-6" dir="rtl">
      {questions.map((question, index) => (
        <motion.div
          ...
          className={`
            p-6 rounded-xl cursor-pointer transition-all text-right
        ...
        >
          ...
          <div className="mt-4">
            ...
            {(question.type === 'dropdown' || question.type === 'checkboxes') && (
              <div className="space-y-2">
                {(question.options || []).map((option, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 p-3 rounded-lg border-2 transition-colors hover:border-opacity-100 flex-row-reverse"
                    style={{
                      borderColor: `${theme.primaryColor}40`,
                      color: theme.textColor
                    }}
            ...
            {question.type === 'yes_no' && (
              <div className="flex gap-3">
                {[
                  ...
                  <div
                    key={i}
                    className="flex items-center gap-3 p-3 rounded-lg border-2 flex-1 justify-center transition-colors flex-row-reverse"
                    style={{
                      borderColor: `${theme.primaryColor}40`,
                      color: theme.textColor
                    }}
            ...
            {question.type === 'file_upload' && (
              ...
              >
                <p className="text-sm">اسحب الملفات هنا أو اضغط للرفع</p>
                <p className="text-xs opacity-60 mt-1">
                  الصور وملفات PDF حتى {question.maxFileSize || 10} ميغابايت
                </p>
              </div>
            )}
          </div>
          ...
          {/* Keyboard hint */}
          <div className="mt-6 flex items-center gap-2 opacity-50 justify-end">
            <kbd
            ...
            <span className="text-xs" style={{ color: theme.textColor }}>اضغط</span>
          </div>
        </motion.div>
      ))}
    </div>
